...
# =====================================================================================
# Top-Level Helper Functions for Dataset Processing (for Multiprocessing)
# =====================================================================================
from functools import partial

def worker_init_fn(worker_id, tokenizer_name):
    """
    Initializes a tokenizer for each worker, which will then be used in the
    processing function applied by each worker.
    """
    # This makes the tokenizer available only within this worker's scope for the map function
    global worker_tokenizer
    print(f"Initializing tokenizer for worker {worker_id}...")
    worker_tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, use_fast=True)
    if worker_tokenizer.pad_token is None:
        worker_tokenizer.pad_token = worker_tokenizer.eos_token
        print(f"Worker {worker_id}: pad_token set to eos_token (ID: {worker_tokenizer.eos_token_id})")

def process_and_tokenize(examples, text_column, sequence_length):
    """
    A single function that normalizes the text field and then tokenizes.
    It will use the `worker_tokenizer` initialized in each process.
    """
    # Step 1: Normalize the text field. This example assumes the 'text' key exists.
    # In a more complex setup, this could use a map like you had before.
    # For simplicity, we'll assume the interleaved dataset yields dicts with a 'text' key.
    text = examples[text_column]

    # Step 2: Tokenize and pad to a fixed length
    if 'worker_tokenizer' not in globals():
         raise RuntimeError("Worker tokenizer not initialized! Check worker_init_fn.")
...
def normalize_text_field(example, text_field_map, name):
    """Normalizes different text column names to a single 'text' field."""
    field = text_field_map.get(name, "text")
    return {"text": example.get(field, "")}

def filter_non_string_and_empty(example):
    """Filters out examples where the 'text' field is not a valid string."""
    return isinstance(example["text"], str) and example["text"].strip() != ""

# def tokenize_and_pad_batch(examples, text_column, sequence_length):
#     """A global function to tokenize a batch of examples with fixed padding."""
#     if TOKENIZER is None:
#         raise RuntimeError("Tokenizer is not initialized in this worker process. Check worker_init_fn.")
#     return TOKENIZER(
#         examples[text_column],
#         truncation=True,
#         padding="max_length", # Fixed padding is crucial here
#         max_length=sequence_length
#     )
...
def select_text_column_fn(example):
    ...
    if "text" in example:
        ...
        logger.warning(
            f"Neither '{"text"}' nor 'text' found in example keys: {list(example.keys())}. "
            f"Returning empty string for '{"text"}'."
        )
        return {"text": ""}

# =====================================================================================
# MappedIterableDataset: A wrapper for IterableDataset to apply a map function
# This is necessary because the Hugging Face Datasets library allows .map() on IterableDatasets
# But PyTorch DataLoader doesn't apply .map(). We need a simple wrapper.
class MappedIterableDataset(torch.utils.data.IterableDataset):
    """
    A wrapper that handles correct sharding for multiple interleaved streaming datasets
    to work with a multi-worker DataLoader.
    """
    def __init__(self, dataset_sources, process_fn):
    ...
    def __iter__(self):
        ...
        if worker_info is None:
            # Not in a worker -> single-process mode
            # Interleave all sources directly
            worker_id = 0
            num_workers = 1
            print(f"Running in single-process mode (worker_id={worker_id}, num_workers={num_workers})")
        else:
            # In a worker -> multi-process mode
            worker_id = worker_info.id
            num_workers = worker_info.num_workers
            print(f"Running in worker {worker_id} of {num_workers}")

        # Each worker will get its own shard from each source dataset
        sharded_sources = [
            d.shard(num_shards=num_workers, index=worker_id) for d in self.sources
        ]
        print(sharded_sources[:5])
        print(f"Worker {worker_id} processing {len(sharded_sources)} sharded sources.")
        
        # Interleave the sharded sources for this worker
        interleaved_dataset = interleave_datasets(sharded_sources, stopping_strategy="all_exhausted")
        
        # print(interleaved_dataset)
        
        # Apply the tokenization map to the final interleaved stream for this worker
        return iter(sharded_sources[0].map(self.process_fn, batched=True, remove_columns=["text"]))
        # return iter(interleaved_dataset.map(self.process_fn, batched=True, remove_columns=["text"]))
...
def prepare_combined_dataset_streaming(sample_sizes=None, save_path=None):
    ...
    if sample_sizes is None:
        sample_sizes = {
            "Skylion007/openwebtext": 800000,
            "allenai/c4": 800000,
            "togethercomputer/RedPajama-Data-1T": 800000,
            "ai4bharat/sangraha": 800000,
            "zicsx/mC4-Hindi-Cleaned-3.0": 800000,
            "PleIAs/common_corpus": 800000
        }
    ...
    for name, limit in sample_sizes.items():
        ...
        non_text_columns = ['timestamp', 'url', 'meta', 'red_pajama_subset', 'doc_id', 'type', 'identifier', 'collection', 'open_type', 'license', 'date', 'title', 'creator', 'language', 'language_type', 'word_count', 'token_count', '__index_level_0__']
        try:
            logger.info(f"🌐 Streaming {name} (limit {limit})...")
            ds_stream = load_dataset(name, **kwargs)
            ds_sample = ds_stream.take(limit)

            # Use a partial function to pass arguments to the global function
            normalizing_function = partial(normalize_text_field, text_field_map=text_field_map, name=name)

            # REPLACED LAMBDAS with the new global functions
            ds_sample = ds_sample.map(normalizing_function, remove_columns = non_text_columns)
            ds_sample = ds_sample.filter(filter_non_string_and_empty)
            
            # ds_sample = ds_sample.shard(
            #     num_shards=4,
            #     index=0  # Shard to reduce memory usage, adjust as needed
            # )
    ...
    combined_iterable = datasets_streamed
    logger.info(f"✅ Prepared a list of {len(datasets_streamed)} streaming datasets.")
    # print(combined_iterable)
    # import sys; sys.exit(0) # Debugging line to stop execution here
...
def main():
    ...
    accelerator = Accelerator(
        split_batches=True,  # Set to False for more control over DataLoader batching
    ...
    # 3. Now it is safe to use the logger and initialize other components
    # logger.info(f"Initializing global tokenizer: {args.tokenizer_name}")
    # _initialize_global_tokenizer(args.tokenizer_name) # Assuming you have this helper function
    ...
    # --- Tokenizer (for Main Process) ---
    logger.info(f"Loading tokenizer '{args.tokenizer_name}' for main process...")
    # This instance is ONLY for the DataCollator and generation steps
    main_process_tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_name, use_fast=True)
    if main_process_tokenizer.pad_token is None:
        main_process_tokenizer.pad_token = main_process_tokenizer.eos_token
        config.pad_token_id = main_process_tokenizer.pad_token_id

    # Resize model embeddings if tokenizer vocab size is different (should match if configured correctly)
    if len(main_process_tokenizer) != model.config.vocab_size:
        logger.warning(
            f"Tokenizer vocab size ({len(main_process_tokenizer)}) does not match model config vocab size ({model.config.vocab_size}). "
            f"Resizing model token embeddings. This is not ideal for 'from scratch' if vocabularies differ significantly."
        )
        model.resize_token_embeddings(len(main_process_tokenizer))
    ...
    if effective_streaming:
        # `raw_datasets` is the combined, filtered, text-normalized stream
        # We will apply the final tokenization map
        tokenizer_with_args = partial(
            process_and_tokenize, # Use the new global function
            text_column=args.text_column, 
            sequence_length=args.sequence_length
        )

        logger.info(f"Applying tokenization with fixed padding to max_length={args.sequence_length}.")
        processed_dataset = MappedIterableDataset(raw_datasets, tokenizer_with_args)
        logger.info(processed_dataset) # Debugging line to check the dataset structure'

    else: # Non-streaming path
        def non_streaming_tokenize_function(examples):
            return main_process_tokenizer(examples[args.text_column], truncation=True, max_length=args.sequence_length, return_special_tokens_mask=True)
    ...
    # --- Create DataLoader ---
    # Prepare the worker init function
    data_loader_worker_init_fn = partial(worker_init_fn, tokenizer_name=args.tokenizer_name)

    train_dataloader = DataLoader(
        processed_dataset, # Pass the wrapped dataset
        shuffle=False,
        collate_fn=DataCollatorForLanguageModeling(tokenizer=main_process_tokenizer, mlm=False),
        batch_size=args.per_device_train_batch_size,
        num_workers=4, # Use a good number of workers (e.g., half your CPU cores)
        worker_init_fn=data_loader_worker_init_fn,
        pin_memory=True
    )
            ...
            if accelerator.sync_gradients:
                ...
                if completed_steps % args.generation_steps == 0 and completed_steps > 0:
                    if accelerator.is_main_process:
                        ...
                        prompt_encoding = main_process_tokenizer(
                        ...
                        try:
                            with torch.no_grad():
                                generated_ids = unwrapped_model.generate(
                                    ...
                                    top_p=0.95,
                                    temperature=0.8,
                                    pad_token_id=main.eos_token_id # Explicitly set pad_token_id for open-end generation
                                )
                            generated_text = main_process_tokenizer.decode(generated_ids[0], skip_special_tokens=True)
                ...
                if completed_steps % args.save_steps == 0 and completed_steps > 0:
                    ...
                    if accelerator.is_main_process:
                        ...
                        # Save tokenizer
                        main_process_tokenizer.save_pretrained(output_checkpoint_dir)
    ...
    if args.output_dir is not None:
        ...
        if accelerator.is_main_process:
            ...
            main_process_tokenizer.save_pretrained(args.output_dir)
