...
# This will be initialized in each worker process by worker_init_fn
TOKENIZER = None

def worker_init_fn(worker_id, tokenizer_name):
    """Initializes a tokenizer and sets the pad_token in each worker process."""
    global TOKENIZER
    print(f"Initializing tokenizer for worker {worker_id}...") # For debugging
    TOKENIZER = AutoTokenizer.from_pretrained(tokenizer_name, use_fast=True)
    if TOKENIZER.pad_token is None:
        TOKENIZER.pad_token = TOKENIZER.eos_token
        print(f"Worker {worker_id}: pad_token set to eos_token (ID: {TOKENIZER.eos_token_id})")
...
def prepare_combined_dataset_streaming(sample_sizes=None, save_path=None):
    ...
    for name, limit in sample_sizes.items():
        ...
        try:
            ...
            ds_sample = ds_sample.map(normalizing_function)
...
def main():
    ...
    model = AutoModelForCausalLM.from_config(
        config,
        attn_implementation="flash_attention_2", # <-- The magic happens here
    ...
    if main_process_tokenizer.pad_token is None:
        ...
        # Also update the model's config to know the pad_token_id
        config.pad_token_id = main_process_tokenizer.pad_token_id
    
    # Llama specific tokenizer settings if not already set
    if main_process_tokenizer.pad_token is None:
        main_process_tokenizer.pad_token = main_process_tokenizer.eos_token # Common practice for Llama-like models
    ...
    if effective_streaming:
        ...
        tokenizer_with_args = partial(
            tokenize_and_pad_batch, # Use the new global function
    ...
    train_dataloader = DataLoader(
        ...
        num_workers=0,  # Re-enable multiprocessing. Start with 4 or 8.
